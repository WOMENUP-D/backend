...
async def has_consent(session: AsyncSession, user_id: uuid.UUID, scope: ConsentScope) -> bool:
    """Current consent state = the most recent entry for that scope."""
    latest = await session.scalar(
        select(ConsentLog.accepted)
        .where(ConsentLog.user_id == user_id, ConsentLog.scope == scope)
...
async def current_consents(session: AsyncSession, user_id: uuid.UUID) -> dict[ConsentScope, bool]:
    """Latest state for every scope the user has ever answered."""
